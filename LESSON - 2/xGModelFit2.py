...
#### Plotting Shots and xG created by top 3 scorers of 2017/18 Premier League season (Salah , Kane , Aguero) ,
#### Q.1 Who underperformed or overperformed their xG over the season?
#### Q.2 Which player had the highest xG in 2017/18 Premier League season?
#### Q.3 which player had the non-penalty highest xG in 2017/18 Premier League season?
#### Q.4 Which player had the highest xG from free kicks?
...
# Opening data
# ----------------------------
# To fit the xG model we will use Wyscout data.
...
shots = train.loc[train['subEventName'].isin(['Shot','Penalty','Free kick shot'])]
...
#pitch = VerticalPitch(line_color='black', half = True, pitch_type='tracab', line_zorder = 2)
...
#shots = train.loc[train['subEventName'].isin(['Shot','Penalty','Free kick shot'])]
shots_salah = shots.loc[shots['playerId'] == 120353]
#add an xG to my dataframe
...
model = model + model_variables[-1]
#fit the model
test_model_salah = smf.glm(formula="Goal ~ " + model, data=shots_salah, 
                           family=sm.families.Binomial()).fit()
#print summary
print(test_model_salah.summary())        
b=test_model_salah.params
...
xGSalah=shots_salah.apply(calculate_xG, axis=1) 
shots_salah = shots_salah.assign(xG=xGSalah)
#Create a 2D map of xG
pgoal_2d2=np.zeros((68,68))
for x in range(68):
    for y in range(68):
        sh2=dict()
        a = np.arctan(7.32 *x /(x**2 + abs(y-68/2)**2 - (7.32/2)**2))
        if a<0:
            a = np.pi + a
        sh2['Angle'] = a
        sh2['Distance'] = np.sqrt(x**2 + abs(y-68/2)**2)
        
        pgoal_2d2[x,y] =  calculate_xG(sh2)

#plot pitch
pitch = VerticalPitch(line_color='black', half = True, pitch_type='custom', pitch_length=105, pitch_width=68, line_zorder = 2)
fig, ax = pitch.draw()
#plot probability
pos = ax.imshow(pgoal_2d2, extent=[-1,68,68,-1], aspect='auto',cmap=plt.cm.Blues,vmin=0, vmax=0.3, zorder = 1)
fig.colorbar(pos, ax=ax)
#make legend
ax.set_title('Probability of goal')
plt.xlim((0,68))
...
shots_kane = shots.loc[shots['playerId'] == 8717]
#add an xG to my dataframe
...
model = model + model_variables[-1]
#fit the model
test_model_kane = smf.glm(formula="Goal ~ " + model, data=shots_kane, 
                           family=sm.families.Binomial()).fit()
#print summary
print(test_model_kane.summary())        
b=test_model_kane.params


xGKane=shots_kane.apply(calculate_xG, axis=1) 
shots_kane = shots_kane.assign(xG=xGKane)
#Create a 2D map of xG
pgoal_2d3=np.zeros((68,68))
for x in range(68):
    for y in range(68):
        sh3=dict()
        a = np.arctan(7.32 *x /(x**2 + abs(y-68/2)**2 - (7.32/2)**2))
        if a<0:
            a = np.pi + a
        sh3['Angle'] = a
        sh3['Distance'] = np.sqrt(x**2 + abs(y-68/2)**2)
        
        pgoal_2d3[x,y] =  calculate_xG(sh3)

#plot pitch
pitch = VerticalPitch(line_color='black', half = True, pitch_type='custom', pitch_length=105, pitch_width=68, line_zorder = 2)
fig, ax = pitch.draw()
#plot probability
pos = ax.imshow(pgoal_2d3, extent=[-1,68,68,-1], aspect='auto',cmap=plt.cm.Blues,vmin=0, vmax=0.3, zorder = 1)
fig.colorbar(pos, ax=ax)
#make legend
ax.set_title('Probability of goal')
plt.xlim((0,68))
...
shots_aguero = shots.loc[shots['playerId'] == 8325]
test_model_aguero = smf.glm(formula="Goal ~ " + model, data=shots_aguero, 
                           family=sm.families.Binomial()).fit()
#print summary
print(test_model_aguero.summary())        
b=test_model_aguero.params


xGAguero=shots_aguero.apply(calculate_xG, axis=1) 
shots_aguero = shots_aguero.assign(xG=xGAguero)
#Create a 2D map of xG
pgoal_2d4=np.zeros((68,68))
for x in range(68):
    for y in range(68):
        sh4=dict()
        a = np.arctan(7.32 *x /(x**2 + abs(y-68/2)**2 - (7.32/2)**2))
        if a<0:
            a = np.pi + a
        sh4['Angle'] = a
        sh4['Distance'] = np.sqrt(x**2 + abs(y-68/2)**2)
        
        pgoal_2d4[x,y] =  calculate_xG(sh4)

#plot pitch
pitch = VerticalPitch(line_color='black', half = True, pitch_type='custom', pitch_length=105, pitch_width=68, line_zorder = 2)
fig, ax = pitch.draw()
#plot probability
pos = ax.imshow(pgoal_2d4, extent=[-1,68,68,-1], aspect='auto',cmap=plt.cm.Blues,vmin=0, vmax=0.3, zorder = 1)
fig.colorbar(pos, ax=ax)
#make legend
ax.set_title('Probability of goal')
plt.xlim((0,68))
