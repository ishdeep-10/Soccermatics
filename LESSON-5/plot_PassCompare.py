...
the degree to which passing/possession lead to more goals being scored and
...
# First we scatter the data. We also want to highlight England Men's team performance in these 2 areas.
...
england_df  = passshot_df.loc[passshot_df["Team"] == "England"]
...
# First we draw 24 pitches, one for each team that played in WC. Then, for each team, we calculate the number of passes in 
...
fig, axs = pitch.grid(ncols = 8, nrows = 4, figheight=20,
