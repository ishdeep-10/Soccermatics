...
We calculate the following metrics directly from
...
Compare other wingers from same league and other leagues using pizza plots
...
image_path = "C:/Users/acer/Pictures/mahrez.png"
...
# ----------------------------
# For this tutorial we decided to use Riyad Mahrez as our player. First, we have to find his 
...
#player to investigate - Riyad Mahrez
#only his statistics
mahrez = summary_per_90.loc[summary_per_90["shortName"] == "R. Mahrez"]
#columns similar together
mahrez = mahrez[['npxG_per90', "goals_per90", "assists_per90", "key_passes_per90", "smart_passes_per90", "final_third_passes_per90", "final_third_receptions_per90", "ground_duels_won_per90", "air_duels_won_per90"]]
#take only necessary columns - exclude playerId
per_90_columns = mahrez.columns[:]
#values to mark on the plot
values = [round(mahrez[column].iloc[0],2) for column in per_90_columns]
#percentiles
percentiles = [int(stats.percentileofscore(summary_per_90[column], mahrez[column].iloc[0])) for column in per_90_columns]
...
# add title
fig.text(
    0.515, 0.97, "Riyad mahrez per 90 - Leichester City", size=18,
...
##############################################################################
# Making the plot with adjusted data for mahrez
# ----------------------------
...
mahrez_adjusted = summary_adjusted.loc[summary_adjusted["shortName"] == "R. Mahrez"]
mahrez_adjusted = mahrez_adjusted[['npxG_adjusted_per90', "goals_adjusted_per90", "assists_adjusted_per90", "key_passes_adjusted_per90", "smart_passes_adjusted_per90", "final_third_passes_adjusted_per90", "final_third_receptions_adjusted_per90", "ground_duels_won_adjusted_per90", "air_duels_won_adjusted_per90"]]
#take only necessary columns
adjusted_columns = mahrez_adjusted.columns[:]
#values
values = [mahrez_adjusted[column].iloc[0] for column in adjusted_columns]
#percentiles
percentiles_adj = [int(stats.percentileofscore(summary_adjusted[column], mahrez_adjusted[column].iloc[0])) for column in adjusted_columns]
...
fig, ax = baker.make_pizza(
    percentiles_adj,              # list of values
    figsize=(10, 10),      # adjust figsize according to your need
    param_location=110,
    slice_colors=slice_colors,
    value_colors = text_colors,
...
# add title
fig.text(
    0.515, 0.97, "Riyad Mahrez per 90 (possesion adjusted) - Leichester City", size=18,
    ha="center",color="#000000"
)
...
# add title
fig.text(
    0.515, 0.975, "Riyad Mahrez - Leichester City", size=16,
