...
DATADIR = 'C:/Users/acer/Documents/GitHub/Soccermatics/LESSON - 6/metrics/sample-data/data'
game_id = 2 # let's look at sample match 1
...
#goal_frames = events[(events['Type'] == 'SHOT') & (events['Subtype'].str.contains('GOAL'))]['Start Frame'].tolist()

# Filtering the DataFrame based on conditions
goal_frames_dict = {}
for index, row in events.iterrows():
    if row['Type'] == 'SHOT' and 'GOAL' in row['Subtype']:
        team_name = row['Team']
        start_frame = row['Start Frame']
        if team_name not in goal_frames_dict:
            goal_frames_dict[team_name] = []
        goal_frames_dict[team_name].append(start_frame)
...
mviz.plot_frame( tracking_home.loc[3675], tracking_away.loc[3675], include_player_velocities=False, annotate=True)
...
# Initialize a dictionary to store the occurrence of each goal for each team
goal_occurrence = {team_name: 1 for team_name in goal_frames_dict.keys()}

for team_name, start_frames in goal_frames_dict.items():
    for start_frame in start_frames:
        clip_name = f"{team_name}_goal_{goal_occurrence[team_name]}"
        #mviz.save_match_clip(tracking_home.iloc[start_frame-1000:start_frame+200],
                             #tracking_away.iloc[start_frame-1000:start_frame+200],
                             #PLOTDIR+'/goal_clips_game_1', fname=clip_name, include_player_velocities=True)
        
        # Increment the occurrence for the current team
        goal_occurrence[team_name] += 1
